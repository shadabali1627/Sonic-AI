...
import { getChats, getCurrentUser, deleteChat } from "@/lib/api"
...
export function Sidebar({ 
    ...
    const confirmDeleteChat = async () => {
        if (deleteChatState.chatId) {
            ...
            try {
                await deleteChat(deleteChatState.chatId);
            } catch (e) {
    ...
    return (
        <>
                ...
                {isOpen && (
                    <>
                        <motion.div
                            ...
                            exit={{ opacity: 0 }}
                            onClick={onClose}
                            className="fixed inset-0 bg-black/85 backdrop-blur-sm lg:hidden z-50"
                        />
                        <motion.aside
                            initial={{ x: "-100%" }}
                            animate={{ x: 0 }}
                            ...
                            className="fixed inset-y-0 left-0 w-[280px] bg-[#080a0e]/75 backdrop-blur-xl border-r border-white/5 z-50 flex flex-col lg:hidden"
                        >
                            {/* Mobile Header */}
                            <div className="flex items-center justify-between p-4 border-b border-white/5">
                                <div className="flex items-center gap-3 font-heading font-black text-xl text-white">
                                ...
                                {Object.entries(groupedChats).map(([label, groupItems]) => (
                                    groupItems.length > 0 && (
                                        <div key={label} className="mb-6">
                                            <div className="px-4 py-2 text-[10px] font-bold text-gray-500 tracking-wider mb-1 uppercase">
                                                {label}
                                            </div>
                                            <div className="space-y-0.5">
                                                {groupItems.map((chat) => {
                                                    const chatUid = chat.id || chat._id;
                                                    const isActive = activeChatId === chatUid;
                                                    return (
                                                        <div key={chatUid} className="relative group/item">
                                                            <Button
                                                                variant="ghost"
                                                                className={cn(
                                                                    "w-full justify-start gap-3 h-10 px-4 font-normal rounded-lg transition-all duration-200",
                                                                    isActive 
                                                                        ? "text-cyan-400 bg-cyan-500/10 border-l-2 border-cyan-500 pl-3 font-medium" 
                                                                        : "text-gray-400 hover:text-white hover:bg-white/5"
                                                                )}
                                                                onClick={() => { onLoadChat(chatUid); onClose(); }}
                                                            >
                                                                <MessageSquare className={cn("h-4 w-4 shrink-0", isActive ? "text-cyan-400" : "text-gray-500")} />
                                    ...
                                    <div className="flex items-center justify-between" onClick={() => router.push('/settings/profile')}>
                                        <div className="flex items-center gap-3">
                                            <div className="relative shrink-0">
                                                <div className="h-9 w-9 rounded-lg bg-linear-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold shadow-lg shadow-cyan-500/20 ring-1 ring-white/10">
                                                    {user.username?.charAt(0).toUpperCase()}
                                                </div>
                                                <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-emerald-500 ring-2 ring-[#080a0e] animate-pulse" />
                                            </div>
                                            <div>
                                        ...
                                        <Settings className="h-5 w-5 text-gray-500 hover:text-white transition-colors cursor-pointer" />
                                    </div>
                                ) : (
            ...
            <motion.aside
                ...
                className="hidden lg:flex flex-col h-full bg-[#080a0e]/45 backdrop-blur-2xl border-r border-white/5 overflow-hidden whitespace-nowrap z-40 relative"
            >
                {/* Header */}
                <div className={cn("flex items-center h-16 shrink-0 px-4 border-b border-white/5", isCollapsed ? "justify-center" : "justify-between")}>
                    <div className={cn("flex items-center gap-3 font-heading font-black text-xl text-white", isCollapsed && "hidden")}>
                        ...
                        <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                            Sonic<span className="text-cyan-400 font-black">AI</span>
                        </motion.span>
                    </div>

                    <Button variant="ghost" size="icon" onClick={onDesktopToggle} className="text-gray-500 hover:text-white hover:bg-white/5 transition-all">
                ...
                <div className="flex-1 flex flex-col overflow-hidden relative">
                    ...
                    {!isCollapsed && (
                        <div className="flex-1 overflow-y-auto px-3 py-2 scrollbar-thin scrollbar-thumb-white/5 scrollbar-track-transparent">
                            {Object.entries(groupedChats).map(([label, groupItems]) => (
                                groupItems.length > 0 && (
                                                ...
                                                return (
                                                    <div key={chatUid} className="relative group/item">
                                                        <Button
                                                            variant="ghost"
                                                            className={cn(
                                                                "w-full justify-start gap-3 h-9 px-3 font-normal rounded-lg text-sm transition-all duration-200",
                                                                isActive 
                                                                    ? "text-cyan-400 bg-cyan-500/10 border-l-2 border-cyan-500 pl-2 font-medium" 
                                                                    : "text-gray-400 hover:text-white hover:bg-white/5"
                                                            )}
                                                            onClick={() => onLoadChat(chatUid)}
                                                        >
                                                            <MessageSquare className={cn("h-4 w-4 shrink-0 transition-all duration-200", isActive ? "text-cyan-400 opacity-100" : "opacity-70 group-hover/item:text-cyan-400 group-hover/item:opacity-100")} />
                                                        ...
                                                        <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover/item:opacity-100 transition-opacity">
                                                            <Button
                                                                ...
                                                                onClick={(e) => toggleMenu(e, chatUid)}
                                                            >
                                                        ...
                                                        {activeMenuId === chatUid && (
                                                            <div ref={menuRef} className="absolute right-0 top-8 z-50 w-32 bg-[#1A1D21] border border-white/10 rounded-lg shadow-xl py-1 animate-in zoom-in-95 duration-100">
                                                                <div
                                                                    ...
                                                                    onClick={(e) => handleRenameChat(e, chatUid, chat.title)}
                                                                >
                                                                ...
                                                                <div
                                                                    ...
                                                                    onClick={(e) => handleDeleteChat(e, chatUid, chat.title)}
                                                                >
                ...
                {/* Footer (Sticky) */}
                <div className={cn("border-t border-white/5 p-4 shrink-0 bg-transparent", isCollapsed && "p-2 flex justify-center")}>
                    {user ? (
                        <div
                            className={cn(
                                ...
                                {!isCollapsed && (
                                    <div className="flex flex-col min-w-0">
                                        ...
                                        <div className="flex items-center gap-1.5 mt-0.5">
                                            <span className="truncate text-[10px] font-bold text-cyan-400 tracking-wider uppercase">Pro Plan</span>
                                        </div>
                                    </div>
                                )}
                            </div>
